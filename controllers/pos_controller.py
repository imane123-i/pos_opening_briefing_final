...
class PosChecklistController(http.Controller):
    ...
    def check_briefing(self, **kw):
        """Vérifie si la checklist a été remplie et l'état de la session"""
        try:
            ...
            # ✅ NOUVEAU : Retourne aussi l'état de la session
            return {
                'briefing_done': session.opening_checklist_done,
                'session_name': session.name,
                'session_state': session.state,  # ← NOUVEAU
                'show_form': (session.state == 'new_session' and not session.opening_checklist_done),  # ← NOUVEAU
            }
