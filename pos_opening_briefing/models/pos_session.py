...
RECIPIENT_EMAIL = "imane.elmoudny@e-polytechnique.ma"
...
class PosSession(models.Model):
    ...
    perf_best_seller = fields.Many2one("product.product", string="Best seller")
    perf_slow_seller = fields.Many2one("product.product", string="Slow seller")
    ...
    # --- CONTACTS RAPPORT ---
    recipient_email = fields.Char("Email destinataire")
    phone_number = fields.Char("Numéro WhatsApp")
    ...
    def _build_email_body(self, session):
        best_str = session.perf_best_seller.display_name if session.perf_best_seller else "Non renseigné"
        slow_str = session.perf_slow_seller.display_name if session.perf_slow_seller else "Non renseigné"
    ...
    def action_save_checklist(self, session_id, vals):
        ...
        try:
            ...
            try:
                ...
                best_seller_name = self.perf_best_seller.display_name if self.perf_best_seller else ""
                slow_seller_name = self.perf_slow_seller.display_name if self.perf_slow_seller else ""
