...
patch(ProductScreen.prototype, {
    ...
    async _onBeforeUnload() {
        ...
        return super._onBeforeUnload && super._onBeforeUnload(...arguments);
    },

    async _checkBriefing() {
        try {
            const sessionData = await this.orm.read("pos.session", [this.pos.session.id], ["briefing_checklist_done"]);
            const briefingDone = sessionData && sessionData[0] && sessionData[0].briefing_checklist_done;
